...
def handle_fs_event(cur: sqlite3.Cursor, data: dict):
    ...
    # ===================== LOG =====================
        ...
        if fd is not None:
            cur.execute("""
            INSERT INTO state (pid, fd_number, file_type, file_ref, file_off, readable, writable, last_update_tick)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(pid, fd_number) DO UPDATE SET
                file_ref = excluded.file_ref,
                file_off = excluded.file_off,
                last_update_tick = excluded.last_update_tick
            """, (data.get("pid"), fd, "FILE", current_ref, current_off, data.get("readable"), data.get("writable"), data.get("tick")))    

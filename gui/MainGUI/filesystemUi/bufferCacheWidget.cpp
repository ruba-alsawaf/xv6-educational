...
void BufferCacheWidget::loadAllEventsFromDB() {
    ...
    const char *sql = R"(
        SELECT seq, tick, fs_type, pid, dev, blockno, old_blockno, buf_id,
               ...
               name, op_name
        FROM fs_events
        WHERE fs_type IN (3, 4, 5, 6, 7)
        ORDER BY seq ASC
    )";

    sqlite3_stmt *stmt;
    if (sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr) != SQLITE_OK) {
        qWarning() << "خطأ في الاستعلام:" << sqlite3_errmsg(db);
        return;
    }
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        event.tick = sqlite3_column_int(stmt, 1);
        event.fs_type = sqlite3_column_int(stmt, 2);
        event.pid = sqlite3_column_int(stmt, 3);
        event.dev = sqlite3_column_int(stmt, 4);
        event.blockno = sqlite3_column_int(stmt, 5);
        event.old_blockno = sqlite3_column_int(stmt, 6);
        event.buf_id = sqlite3_column_int(stmt, 7);
        event.ref_before = sqlite3_column_int(stmt, 8);
        event.ref_after = sqlite3_column_int(stmt, 9);
        event.valid_before = sqlite3_column_int(stmt, 10);
        event.valid_after = sqlite3_column_int(stmt, 11);
        event.locked_before = sqlite3_column_int(stmt, 12);
        event.locked_after = sqlite3_column_int(stmt, 13);
        event.lru_before = sqlite3_column_int(stmt, 14);
        event.lru_after = sqlite3_column_int(stmt, 15);
        event.name = QString::fromUtf8((const char *)sqlite3_column_text(stmt, 16));
        event.op_name = QString::fromUtf8((const char *)sqlite3_column_text(stmt, 17));

        events.append(event);

        // إضافة إلى قائمة الأحداث
        QString listItem = QString("#%1 [%2] %3 - %4")
            .arg(event.seq)
            .arg(event.tick)
            .arg(event.op_name)
            .arg(event.name);
    ...
    if (events.isEmpty()) {
        currentEventLabel->setText("❌ لم يتم العثور على أحداث في قاعدة البيانات");
        return;
    }
...
QMap<int, BufferState> BufferCacheWidget::getBufferStatesAtSequence(long seq) {
    ...
    // احصل على آخر حالة لكل بفر قبل أو عند هذا الـ Sequence
    const char *sql = R"(
        SELECT DISTINCT buf_id FROM buffer_state
    )";
    ...
    // حالياً، سنستخدم آخر حالة معروفة من جدول buffer_state
    // في المستقبل، يمكن تحسينها للحصول على الحالة التاريخية
    const char *sql2 = "SELECT buf_id, blockno, refcnt, valid, locked, lru_pos FROM buffer_state";
    sqlite3_stmt *stmt2;

    if (sqlite3_prepare_v2(db, sql2, -1, &stmt2, nullptr) == SQLITE_OK) {
        while (sqlite3_step(stmt2) == SQLITE_ROW) {
            BufferState state;
            state.buf_id = sqlite3_column_int(stmt2, 0);
            state.blockno = sqlite3_column_int(stmt2, 1);
            state.refcnt = sqlite3_column_int(stmt2, 2);
            state.valid = sqlite3_column_int(stmt2, 3);
            state.locked = sqlite3_column_int(stmt2, 4);
            state.lru_pos = sqlite3_column_int(stmt2, 5);
